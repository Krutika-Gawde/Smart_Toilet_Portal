...
<h3  style="text-align: center;font-weight:bold;">FETCH YOUR DATA</h3>
	...
	<form action="creditretrive.php" method="post">
	...
	<!--<b><label class="col-sm-4 control-label">AREA_ASSIGNED</label></b>
	...
	<!--<b>	<label class="col-sm-4 control-label">TOILET_ASSIGNED</label></b>
	<div class="col-lg-4"></div>
...
if(isset($_POST['submit'])){
		...
		if( $RFID!="" AND $T1DATE!="" AND $F1DATE!="")
	{
			 ...
			 $query=("SELECT * FROM register,credit WHERE(DATE2>='$F1DATE'   AND  DATE2<='$T1DATE' AND register.RFID=credit.RFID1 AND RFID='$RFID')");
			$result=mysqli_query($conn,$query);
			// echo json_encode(array($result));
			$query1=("SELECT NAME,CONTACT,CREDIT,WORKER_ID,DATE2 FROM register,credit where(DATE2>='$F1DATE'   AND  DATE2<='$T1DATE' AND  register.RFID=credit.RFID1 AND RFID='$RFID')");
        	...
        	// $json1 = json_encode($row1); 
       		 ...
       		 $query2=("SELECT AVG(CREDIT) AS CREDIT FROM register,credit where(DATE2>='$F1DATE'   AND  DATE2<='$T1DATE' AND  register.RFID=credit.RFID1 AND RFID='$RFID')");
...
//}
			...
			//	}
				...
				?>
				<h3>TOTAL CREDIT POINTS GAINED = <?php echo $data2['CREDIT']; ?></h3>
				<?php
			}
		}
	?>
